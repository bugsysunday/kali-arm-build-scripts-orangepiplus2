...
dd if=/dev/zero of=${basedir}/kali-$1-orangepi2plus.img bs=1M count=3800
parted kali-$1-orangepi2plus.img --script -- mklabel msdos
parted kali-$1-orangepi2plus.img --script -- mkpart primary fat32 2048s 264191s
parted kali-$1-orangepi2plus.img --script -- mkpart primary ext4 264192s 100%
# Set the partition variables
loopdevice=`losetup -f --show ${basedir}/kali-$1-orangepi2plus.img`
...
git clone https://github.com/OrangePiLibra/OrangePiH3_kernel.git ${basedir}/kernel
git clone https://github.com/OrangePiLibra/OrangePiH3_uboot.git ${basedir}/uboot
git clone https://github.com/OrangePiLibra/OrangePiH3_scripts.git ${basedir}/scripts
git clone https://github.com/OrangePiLibra/OrangePiH3_external.git ${basedir}/external
wget https://codeload.github.com/OrangePiLibra/OrangePiH3_toolchain/zip/master
unzip  ${basedir}/OrangePiH3_toolchain.zip
# Github
...
make -j  ARCH=arm CROSS_COMPILE=${cross_comp}- mrproper > /dev/null 2>&1
make -j  ARCH=arm CROSS_COMPILE=${cross_comp}- sun8iw7p1smp_linux_defconfig  > /dev/null 2>&1
fi
sleep 1
# build kernel (use -jN, where N is number of cores you can spare for building)
echo -e "\e[1;31m Building Kernel and Modules \e[0m"
make -j ARCH=arm CROSS_COMPILE=${cross_comp}- uImage > /dev/null 2>&1
#==================================================
# copy uImage to output
cp arch/arm/boot/uImage ${basedir}/bootp/uImage_OPI-PLUS > /dev/null 2>&1
make -j ARCH=arm CROSS_COMPILE=${cross_comp}- modules > /dev/null 2>&1
sleep 1
echo -e "\e[1;31m Exporting Modules \e[0m"
make -j ARCH=arm CROSS_COMPILE=${cross_comp}- INSTALL_MOD_PATH=${basedir}/rootp modules_install > /dev/null 2>&1
echo -e "\e[1;31m Exporting Firmware ... \e[0m"
make -j ARCH=arm CROSS_COMPILE=${cross_comp}- INSTALL_MOD_PATH=${basedir}/rootp firmware_install > /dev/null 2>&1
sleep 1
# build mali driver
##########################
SCRIPT_DIR=`pwd`
...
# soon.
echo "Generating sha1sum of kali-$1-orangepi2plus.img"
sha1sum kali-$1-orangepi2plus.img > ${basedir}/kali-$1-orangepi2plus.img.sha1sum  > /dev/null 2>&1
## Don't pixz on 32bit, there isn't enough memory to compress the images.
MACHINE_TYPE=`uname -m`
if [ ${MACHINE_TYPE} == 'x86_64' ]; then
echo "Compressing kali-$1-orangepi2plus.img"
pixz ${basedir}/kali-$1-orangepi2plus.img ${basedir}/kali-$1-orangepi2plus.img.xz
rm ${basedir}/kali-$1-orangepi2plus.img
echo "Generating sha1sum of kali-$1-orangepi2plus.img.xz"
sha1sum kali-$1-orangepi2plus.img.xz > ${basedir}/kali-$1-orangepi2plus.img.xz.sha1sum
fi